...
| **多图层编辑** | 支持多图层导入、选中、拖拽移动、显示/隐藏、删除、重排、清空图层 |
| **图层栏** | 右侧图层清单支持拖拽排序（越靠下越在上层），可一键显示/隐藏整个图层栏 |
...
| **雪碧图模式** | 支持按图层顺序生成雪碧图、设置每行帧数、画布雪碧图视图、逐帧播放预览与导出 |
...
- 菜单栏 **文件 → 添加图片为图层**，或快捷键 `Cmd/Ctrl+Shift+O`
- 菜单栏 **文件 → 清空所有图层**，可一键重置画布
- 右侧图层栏支持：
  - 点击选择图层
  - 勾选/取消勾选图层可见性
  - 拖拽排序（列表越靠下，图层越在上层）
- 在画布使用「◫ 图层」工具可直接拖拽图层位置
...
1. 点击左侧工具栏「🎞 雪碧图」进入雪碧图模式
2. 底部面板设置“每行帧数”
3. 画布会按图层顺序显示雪碧图排布
4. 点击「▶ 预览播放」可逐帧预览动画（支持调整播放间隔，默认 500ms）
5. 点击「↓ 导出雪碧图」可直接导出雪碧图
6. 点击「✕ 关闭」退出雪碧图模式并恢复普通画布显示
...
|------|-------|---------|
| 打开 | `Cmd+O` | `Ctrl+O` |
| 添加图片为图层 | `Cmd+Shift+O` | `Ctrl+Shift+O` |
| 清空所有图层 | `Cmd+Shift+Delete` | `Ctrl+Shift+Delete` |
| 导出 | `Cmd+S` | `Ctrl+S` |
| 退出 | `Cmd+Q` | `Ctrl+Q` |
| 撤销 | `Cmd+Z` | `Ctrl+Z` |
...
│   ├── layer_panel.py         # 图层清单面板（选择、显隐、排序）
│   ├── sprite_panel.py        # 雪碧图底部面板（每行帧数、预览、导出）
│   └── dialogs.py             # 对话框（JPEG 质量、缩放、AI 参数、雪碧图预览）
├── controller/
