...
Phase 13: 多图层系统（图层栏、排序、显隐、撤销快照）
Phase 14: 雪碧图模式（画布排布、播放预览、导出）
```
...
以下能力已在当前代码实现中落地，继续开发时请以当前实现为基线，不要按旧版“单图层”假设回退：

1. 多图层数据模型（`ImageModel.layers`，底层到顶层）
2. 右侧图层栏（选择、显隐、拖拽重排、删除）
3. 完整状态快照撤销/重做（含图层结构）
4. 雪碧图模式（每行帧数、画布雪碧图显示、预览播放、导出）
5. 预览播放默认间隔 `500ms`，支持手动调整

如与旧步骤冲突，请优先遵循：
- `docs/PRD.md`（v1.2）
- `docs/TECH_DESIGN.md`（v1.2）
- `docs/MODULE_API.md`（当前实现）
